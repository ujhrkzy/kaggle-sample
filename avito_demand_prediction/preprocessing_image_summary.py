...
class ImageFeatureExtractor(object):
    ...
    def __init__(self, image_name: str, image_directory: str):
        ...
        cmd = "sshpass -p ujihiro0611 scp -rp -P 22 ujihiro@dev.unibo.info:/mnt/ssd/ujihiro/kaggle/res/data/competition_files/test_jpg/{} ./"
        cmd = cmd.format(self._image_name)
        subprocess.call(cmd.split(" "))

        # path = os.path.join(image_directory, image_name)
        path = self._image_name
        self._path = path
    ...
    def extract(self) -> ImageFeatures:
        ...
        cmd = "rm {}".format(self._image_name)
        subprocess.call(cmd.split(" "))
        return features
...
class ImagePreprocessor(object):
    ...
    def execute(self):
        ...
        # train_df = pd.read_csv(data_directory + "test.csv")
        train_df = pd.read_csv("test.csv")
        # train_df = pd.read_csv("top1000_train.csv")

        valid_columns = ["item_id", "image"]
        invalid_columns = [name for name in train_df.columns.values if name not in valid_columns]
        ...
        import math
        count = math.ceil(train_df.shape[0] / 1000)
        for i in range(count):
            if i < 445:
                continue
            sub_train_df = train_df.iloc[i*1000:(i+1)*1000, :]
            sub_train_df["features"] = sub_train_df["image"].apply(_extract_train_feature)
            sub_train_df = self._convert(sub_train_df)
            sub_train_df.to_csv("preprocessed_image_test_{}.csv".format(i), index=False)
            del sub_train_df
            gc.collect()
